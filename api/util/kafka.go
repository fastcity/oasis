...
type KaInterface interface {
	...
	SendMsg(string, string, interface{}) (int32, int64, error)
	ReciveMsg(chan []byte)
...
func NewConsumer(addrs []string) KaInterface {
	...
	logs.Debug("m NewConsumer", m.consumer)
	return m
}
...
func (k *kaModel) SendMsg(key, topic string, data interface{}) (int32, int64, error) {
	...
	logs.Debug("分区ID:%v, offset:%v \n", pid, offset)
	return pid, offset, nil
}

func (k *kaModel) ReciveMsg(msgValue chan []byte) {
	if k.consumer == nil {
		logs.Error("sarama.NewConsumer  k.Consumer==nil:")
	}
	kaTopics, err := k.consumer.Topics()
	if err != nil {
		logs.Error("get topics error")
	}
	...
	for _, topic := range k.topics {
		logs.Debug("topic", topic)

		if !k.topicExist(topic) {
			logs.Error("topic is not exist on kafka", topic)
			continue
		}
		//  一个 topic 一个 协程
		...
		if err != nil {
			logs.Error(" get topic Partitions error:", err)
		}
		...
		for partition := range partitionList {
			...
			if err != nil {
				logs.Error("sarama ConsumePartition error:", err)
			}
